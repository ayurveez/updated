import { Proff, AccessCode, StudentPermissions, CheckUserResponse, SendOTPResponse, VerifyOTPResponse, VerifyCodeResponse } from "../types";
...
export const dataService = {
  ...
  checkUserExists: async (email: string): Promise<CheckUserResponse> => {
    try {
      if (!APPS_SCRIPT_URL) {
  ...
  verifyRegistrationCode: async (code: string): Promise<VerifyCodeResponse> => {
    try {
      if (!APPS_SCRIPT_URL) {
