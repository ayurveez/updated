#!/usr/bin/env node
/**
 * Usage: node scripts/migrate_local_codes.js ./local_codes.json
 * The JSON should be an array of objects with fields matching `AccessCode` in `src/types.ts`.
 * This script inserts hashed-only rows into Supabase assigned_codes table.
 */
const fs = require('fs');
...
if (!file) {
  console.error('Usage: node scripts/migrate_local_codes.js ./local_codes.json');
  process.exit(1);
}
